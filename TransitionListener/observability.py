...
The calculations work as presented in 1811.11175. It is possible to perform the calculation of signal-to-noise ratios for the detector sensitivities provided in
...
class GW_spectrum:
	'''
	Class used to import the sensitivity data curves of GW observatories.
	...
	self.det_names : string tuple
		Detector names, ordered as in input file
...
class GW_analysis_one_point(GW_spectrum):
	...
	self.g_eff_SM : float
		Effective, relativistic dof in the SM at the nucleation of the FOPT
	self.g_eff_DS : float
	...
	def plot_spectrum(self, showplot=True):
		...
		for (name, data, color) in zip(self.det_names, self.det_PLI_data, self.det_colors):       # plotting the detector constraints
			ax.plot(self.det_PLI_f, data, color = color, label = name)
			ax.fill_between(self.det_PLI_f, data,1, alpha = transparency, color = color)
		...
		plt.close(fig)
		xdata = frequencies_Hz
		ydata = self.h2Omega_0(self, frequencies_Hz)
		return xdata, ydata
